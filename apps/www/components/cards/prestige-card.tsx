...
import { canPrestige, formatPrestigeNumber, getPrestigeEstimates, getPrestigeProgress, getPrestigeRequirement } from "@/lib/utils/prestige-client";
import Image from "next/image";
import { PrestigeConfirmationDialog } from "../dialogs/prestige-confirmation-dialog";

export const PrestigeCard = () => {
  const { gameState, performPrestige } = useGameContext();

  if (!gameState) return null;

  if (gameState.prestige_level >= 50) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-yellow-700 dark:text-yellow-300">
                {formatPrestigeNumber(currentMultiplier)}x Multiplier
              </p>
            </div>
          </div>
  ...
  const requirement = getPrestigeRequirement(gameState);
  ...
  const progressPercent = getPrestigeProgress(gameState);
          ...
          <div>
            ...
            {canDoPrestige ? (
              <p className="text-xs text-purple-700 dark:text-purple-300">
                Bonus: {formatPrestigeNumber(estimates.currentMultiplier)}x → {formatPrestigeNumber(estimates.newMultiplier)}x
              </p>
            ) : (
              <div className="-mt-1">
                <PowerTag imageProps={{ width: 10, height: 10, className: "mb-0.5 ml-1 neutralscale" }}>
                  <span className="text-xs">Need {formatPrestigeNumber(requirement)}</span>
                </PowerTag>
